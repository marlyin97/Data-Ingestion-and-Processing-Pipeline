...
#bash ~/sfm_insuredata.sh https://s3.amazonaws.com/inc.data/insurance/insuranceinfo.csv
...
<<steps
1. Formal way of writing shell scripts (shebang,comments, debugging, placeholder $#, $?, $0, $1, $2 ..) - refer the script learn_shell_we43.sh
...
if [ $# -ne 1 ]
then
echo "`date` missing the expected arguments, usage: bash /home/hduser/sfm_insuredata.sh https://s3.amazonaws.com/inc.data/insurance/insuranceinfo.csv"
echo "`date` missing the expected arguments, usage: bash /home/hduser/sfm_insuredata.sh https://s3.amazonaws.com/inc.data/insurance/insuranceinfo.csv" > /tmp/sfm_${dt}.log
exit 10
fi
...
exit #later we will comment this exit

#DATA LOAD FROM HDFS DATALAKE TO HIVE LAKEHOUSE
...
then
echo "Procceding further to create the lakehouse for the Downstream systems consumption or dicovery"
#hive -e "alter table custpart add partition(datadt='$dt')"
